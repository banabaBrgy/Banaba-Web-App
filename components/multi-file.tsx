...
const variants = {
  base: "relative rounded-md p-4 max-w-[calc(100vw-1rem)] flex justify-center items-center flex-col cursor-pointer border-2 border-dashed border-gray-400 dark:border-gray-300 transition-colors duration-200 ease-in-out",
  active: "border-2",
  disabled:
    ...
    const {
      ...
      onDrop: (acceptedFiles) => {
        ...
        if (files) {
          ...
          void onChange?.([...addedFiles, ...(value ?? [])]);
        }
      },
      ...dropzoneOptions,
    });
      ...
      <div className="flex-1">
        <div className="flex flex-col gap-2">
          ...
          {value?.map(({ file, progress }, i) => (
            <div
              key={i}
              className="flex h-16 flex-col justify-center rounded border border-gray-400 px-4 py-2"
            >
